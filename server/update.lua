if Config.CheckForUpdates then
    Citizen.CreateThread( function()
        updatePath = "/MaDHouSe79/qb-savezones"
        resourceName = "("..GetCurrentResourceName()..")"
        PerformHttpRequest("https://raw.githubusercontent.com"..updatePath.."/master/version", checkVersion, "GET")
    end)
end

RegisterServerEvent("qb-savezones:CheckVersion") 
AddEventHandler("qb-savezones:CheckVersion", function()
    if updateavail then
        TriggerClientEvent("qb-savezones:client:Update", source, true)
    else
        TriggerClientEvent("qb-savezones:client:Update", source, false)
    end
end)
